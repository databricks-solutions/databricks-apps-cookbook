...
def layout():
    return dbc.Container([
        ...
        html.Div([
            ...
            html.P("Connect to external APIs and remote MCP servers using Databricks Unity Catalog (UC) connections. This feature allows you to query external service data or remote MCP servers like Slack, Jira, Gmail, Github or any service with an API using HTTP requests. ", className="text-muted mb-4"),
                ...
                dbc.Card([
                    dbc.CardBody([
                        dbc.Row([
                            ...
                            dbc.Col([
                                dbc.Label("Request Type", className="form-label"),
                                dcc.Dropdown(
...
```""",
                ...
                html.H4("Github MCP Server with Oauth On-Behalf-Of User", className="mb-1"),
                dcc.Markdown(
                    """```python
import contextvars
import os
...
```""",
            ...
            dbc.Tab([
                dbc.Row([
                    ...
                    dbc.Col(md=4, children=[
                        html.H5("Compute Requirements", className="mb-2"),
                        html.Ul([
                            html.Li("Databricks Runtime 15.4 LTS or above"),
                            html.Li("Standard or Dedicated access mode"),
                            html.Li("SQL warehouses: Pro or Serverless (2023.40+)"),
                            html.Li("Network connectivity to target systems")
                        ])
                    ]),
                    dbc.Col(md=4, children=[
...
@callback(
    ...
    [Input("connection-select", "value"),
     Input("request-type-select", "value"),
     Input("method-select", "value"),
     Input("path-input", "value"),
     Input("auth-type-select", "value")],
    prevent_initial_call=True
)
def initialize_client(connection, request_type, method, path, auth_type):
...
def send_external_request(n_clicks, connection, request_type, method, path, headers, body, auth_type):
    ...
    try:
        ...
        if w is not None:
            ...
            if request_type == "mcp":
                ...
                headers_dict["Mcp-Session-Id"] = mcp_session_id
...
def retry_mcp_request(n_clicks, connection, request_type, method, path, headers, body, auth_type):
    ...
    return send_external_request(n_clicks, connection, request_type, method, path, headers, body, auth_type) 
...
def retry_main_request(n_clicks, connection, request_type, method, path, headers, body, auth_type):
